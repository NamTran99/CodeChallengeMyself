...
import com.example.ads.activity.format.native_ads.IKSdkBaseNativeFullScreenAds
...
class NativeFullScreenMAX : IKSdkBaseNativeFullScreenAds<IkObjectNativeMax>(
    AdNetwork.AD_MAX
) {
...
//        }
        ...
        if (!isLoadAndShow && checkLoadSameAd(
                idAds.adPriority ?: 0, idAds.cacheSize ?: 0, IKSdkDefConst.TimeOutAd.NATIVE
            )
        ) {
            ...
            object : IKSdkLoadCoreAdCallback<IkObjectNativeMax> {
                override fun onLoaded(result: IkObjectNativeMax) {
                    ...
                    handleTimeout?.onLoaded(
                        this@NativeFullScreenMAX, coroutineScope, objectAd, scriptName
                    )
                    handleTimeout = null
                }

                override fun onLoadFail(error: IKAdError) {
                    handleTimeout?.onLoadFail(this@NativeFullScreenMAX, error, scriptName)
                    handleTimeout = null
                }
            }
