...
class RewardFairBid : IKSdkBaseRewardAd<IKCustomEventData>(AdNetwork.AD_FAIR_BID) {
    ...
    override fun showAd(
        ...
        coroutineScope.launchWithSupervisorJob(Dispatchers.Main) {
            ...
            dataAds.filter { Rewarded.isAvailable(it.getUnitValue()) }.forEach {
            ...
            if (!Rewarded.isAvailable(unitId)) {
                showLogD("not valid Ad")
                adsListener.onAdShowFailed(
                    adNetworkName,
                    screen,
            ...
            adReady?.listener = object : IKAdActionCallback<IKCustomEventData, Any> {
                ...
                override fun onAdImpression(impressionData: Any?) {
                    adsListener.onAdImpression(
                        adNetworkName, screen, "", adReady?.uuid ?: ""
                    )
    ...
    override suspend fun loadCoreAd(
        ...
        Rewarded.setRewardedListener(object : RewardedListener {
            ...
            override fun onCompletion(placementId: String, userRewarded: Boolean) {
                objectAd?.listener?.onAdRewarded(
                    adNetworkName
                )
            }
