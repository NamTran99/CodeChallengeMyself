...
import com.example.ads.activity.mediation.admob.NativeFullScreenAdmob
import com.example.ads.activity.mediation.applovin.NativeFullScreenMAX
import com.example.ads.activity.mediation.gam.NativeFullScreenGam
...
object IKNativeFullScreenController : IKSdkBaseAdController() {
    private const val TAG_LOG = "IKNativeController_"
    override val adMax: NativeFullScreenMAX by lazy {
        NativeFullScreenMAX()
    }

    override val admob: NativeFullScreenAdmob by lazy {
        NativeFullScreenAdmob()
    }

    override val adGam: NativeFullScreenGam by lazy {
        NativeFullScreenGam()
    }
    ...
    override suspend fun getBackupAd(): IKAdapterDto? = IKDataCoreManager.getOtherNativeFullAds()
    override val adFormat: IKAdFormat = IKAdFormat.NATIVE_FULL

    override suspend fun getAdDto(): IKSdkBaseDto? {
        repeat(4) {
            IKDataRepository.getInstance().getSDKNativeFullScreen()?.let {
                mAdDto = it
                return it
            }
            delay(500)
    ...
    suspend fun isAdReady(
        ...
        return when {
            admob.isAdReady(targetPriority, IKSdkDefConst.TimeOutAd.NATIVE_FULL) -> {
                true
            }

            adGam.isAdReady(targetPriority, IKSdkDefConst.TimeOutAd.NATIVE_FULL) -> {
                true
            }

            adMax?.isAdReady(targetPriority, IKSdkDefConst.TimeOutAd.NATIVE_FULL) == true -> {
                true
            }
