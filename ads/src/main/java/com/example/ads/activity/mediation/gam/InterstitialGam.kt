...
class InterstitialGam :
    IKSdkBaseInterstitialAds<AdManagerInterstitialAd>(AdNetwork.AD_MANAGER) {
    override fun showAd(
    ...
    override suspend fun loadCoreAd(
        ...
        callback: IKSdkAdCallback<AdManagerInterstitialAd>
    ) {
        showLogD("loadCoreAd pre start")
//        val context = IKSdkApplicationProvider.getContext()
//        if (context == null) {
//            callback.onAdFailedToLoad(adNetworkName,
...
//        }
        ...
        var handleTimeout: IKSdkHandleTimeoutAd<AdManagerInterstitialAd>? =
            IKSdkHandleTimeoutAd(adNetworkName, idAds, callback)
        var objectAd: IKSdkBaseLoadedAd<AdManagerInterstitialAd>?
        val callbackLoad = object : AdManagerInterstitialAdLoadCallback() {

            override fun onAdFailedToLoad(p0: LoadAdError) {
                super.onAdFailedToLoad(p0)
                showLogD("loadCoreAd onAdFailedToLoad, $p0")
                handleTimeout?.onLoadFail(this@InterstitialGam, IKAdError(p0), scriptName)
                handleTimeout = null
            }
            ...
            override fun onAdLoaded(p0: AdManagerInterstitialAd) {
                ...
                handleTimeout?.onLoaded(this@InterstitialGam, coroutineScope, objectAd, scriptName)
                handleTimeout = null
            }
        }
...
//            )
    ...
    private fun setupOnPaidEventListener(
        adsObject: AdManagerInterstitialAd?, screen: String
    ) = OnPaidEventListener {
